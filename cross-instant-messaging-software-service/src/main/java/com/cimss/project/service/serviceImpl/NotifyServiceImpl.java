...
public class NotifyServiceImpl implements NotifyService {
    ...
    public EventBean.ReplyEvent notify(String groupId, EventBean event) {
        ...
        for(DeliveryMode deliveryMode:deliveryModes){
            switch (deliveryMode){
                ...
                case RABBITMQ-> rabbitMQSendEvent(group.getDeliveryConfig().getRabbitMQConfig(),event);
            }
        }
        return replyEvent;
    }
    ...
    public EventBean.ReplyEvent webhookSendEvent(DeliveryMode.DeliveryConfig.WebhookConfig webhookConfig, EventBean event) {
        ...
        if(webhookConfig.getHeaders()!=null)
            webhookConfig.getHeaders().forEach((key, value) -> headers.add(key, value));
    ...
    public void rabbitMQSendEvent(DeliveryMode.DeliveryConfig.RabbitMQConfig rabbitMQConfig, EventBean event) {
        rabbitTemplate.convertAndSend(rabbitMQConfig.getTopic(),EventBean.CreateMQEvent(rabbitMQConfig.getMetadata(),event));
    }
